...
class CalendarNavigation(QWidget):
    ...
    def apply_style(self, style: CalendarStyleConfig) -> None:
        ...
        self._update_nav_icons(enabled=True)
    ...
    def set_navigation_enabled(self, *, previous_enabled: bool, next_enabled: bool) -> None:
        ...
        self._update_nav_icons(enabled=next_enabled)

    def _update_nav_icons(self, *, enabled: bool) -> None:
        nav_color = self._style.nav_icon_color if enabled else self._style.muted_day_text_color
        prev_icon = load_colored_svg_icon(NAV_LEFT_ICON_PATH, 32, nav_color)
        next_icon = load_colored_svg_icon(NAV_RIGHT_ICON_PATH, 32, nav_color)
        ...
        self._next_button.setCursor(cursor)

    def _create_nav_button(self, _text: str) -> QPushButton:
