...
class ColorPalette:
    ...
    All values are validated hex strings (``#RRGGBB`` or ``#RRGGBBAA``) so
    embedders can feed partially trusted configuration data into
    :class:`Theme` and rely on :func:`validate_hex_color` for sanitisation.
    """
...
class ButtonStyleConfig:
    ...
    Instances are generated by :class:`StyleRegistry` and fed into widgets by
    :class:`StyleManager`. Keeping the configuration immutable ensures styles
    remain deterministic across repaint cycles.
    """
...
class CalendarStyleConfig:
    ...
    These values intentionally mirror :class:`ColorPalette` so palette swaps can
    flow through without touching widget code.
    """
...
class LayoutConfig:
    ...
    Every integer is validated via :func:`validate_dimension` to guard against
    negative or zero-sized widgets.
    """
...
class Theme:
    ...
    ``Theme`` simply holds a :class:`ColorPalette` and :class:`LayoutConfig`,
    validating their types so downstream consumers can rely on consistent
    shapes.
    """
